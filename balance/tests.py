...
class BalanceMathTests(TestCase):
    ...
    def test_contractor_auto_creation_and_normalization(self):
        # ساخت تراکنش با حروف عربی
        WarehouseTransaction.objects.create(
            transaction_type='OUT',
            material=self.material,
            quantity=Decimal('100'),
            contractor_first_name="علي",
            contractor_last_name="رضايي",
            contract_number="123",
            date=date.today()
        )
        
        # باید پیمانکار با حروف فارسی ساخته شود
        contractor = Contractor.objects.first()
        self.assertIsNotNone(contractor)
        self.assertEqual(contractor.first_name, "علی")
        self.assertEqual(contractor.last_name, "رضایی")
        
        # ثبت دوم با همان نام نباید پیمانکار جدید بسازد
        WarehouseTransaction.objects.create(
            transaction_type='OUT',
            material=self.material,
            quantity=Decimal('50'),
            contractor_first_name="علی",
            contractor_last_name="رضایی",
            contract_number="123",
            date=date.today()
        )
